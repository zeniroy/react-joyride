...
function getStyleComputedProperty$1(element) {
...
function getOffsetRectRelativeToArbitraryNode(children, parent) {
  ...
  var scrollParent$$1 = getScrollParent$1(children);
  ...
  if (isIE10 && !fixedPosition ? parent.contains(scrollParent$$1) : parent === scrollParent$$1 && scrollParent$$1.nodeName !== 'BODY') {
...
function getReferenceOffsets(popper, reference) {
...
function (_React$Component) {
  ...
  function JoyrideOverlay(props) {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "fixTop", function () {
      if (_this.spotlight.current) {
        var spotlightElements = _this.spotlight.current.getElementsByClassName("react-joyride__spotlight");

        if (spotlightElements.length > 0) {
          var spotlightElement = spotlightElements[0];
          var element = getElement(_this.props.target);

          if (!isFixed(element)) {
            var offset = getReferenceOffsets(_this.spotlight.current, element);
            spotlightElement.style.top = "".concat(offset.top - 10, "px");
          }
        }
      }
    });

    _this.spotlight = React__default.createRef();
    _this.state = {
